...
//---------------------------------------------------------------------------------------------------------------------	    
	...
	public static String getPaging3(String pageURL, String searchFilter, int nowPage, int rowTotal, int blockList,
			int blockPage) {
		...
		// lab 스타일 페이지네이션
		sb = new StringBuffer("<div class='lab-pagination'>");

		// 이전
		if (isPrevPage) {
			sb.append(String.format(
					"<button type='button' class='lab-page lab-page-prev' "
							+ "onclick=\"location.href='%s?page=%d&%s'\">◀</button>",
					pageURL, (startPage - 1), searchFilter));
		}

		// 페이지 번호
		for (int i = startPage; i <= endPage; i++) {
			if (i == nowPage) {
				sb.append(String.format("<button type='button' class='lab-page is-active'>%d</button>", i));
			} else {
				sb.append(String.format(
						"<button type='button' class='lab-page' "
								+ "onclick=\"location.href='%s?page=%d&%s'\">%d</button>",
						pageURL, i, searchFilter, i));
			}
		}
		...
		if (isNextPage) {
			sb.append(String.format(
					"<button type='button' class='lab-page lab-page-next' "
							+ "onclick=\"location.href='%s?page=%d&%s'\">▶</button>",
					pageURL, (endPage + 1), searchFilter));
		}
