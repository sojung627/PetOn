...
<main class="layout-main pt-20">
  <div class="max-w-4xl mx-auto px-4 py-8">
    ...
    <div class="flex items-center gap-2 text-sm text-gray-500 mb-6">
      ...
      <span class="font-bold text-gray-900">게시글 수정</span>
    </div>

    <form action="${pageContext.request.contextPath}/${b_type}/modify.do"
          method="post"
      ...
      <input type="hidden" name="board_tag" id="board_tag" value="${vo.board_tag}"/>
      ...
      <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
        ...
        <div class="flex justify-between items-start mb-8 pb-6 border-b border-gray-100">
          <div>
            <h1 class="text-2xl font-extrabold text-gray-900 mb-2">게시글 수정</h1>
          ...
          <div>
            ...
            <textarea name="board_content"
                      ...
                      class="w-full px-4 py-4 rounded-xl border border-gray-200
                             focus:outline-none focus:ring-2 focus:ring-amber-400
                             font-medium text-gray-900 h-64 resize-none leading-relaxed"><c:out value="${vo.board_content}" escapeXml="false"/></textarea>
          </div>
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    const currentTag = '${vo.board_tag}';
    document.querySelectorAll('.tag-btn').forEach(function(btn) {
      if (btn.getAttribute('data-value') === currentTag) {
  ...
  function validatePostForm() {
    ...
    CKEDITOR.instances.board_content.updateElement();
...
<script type="text/javascript">
  // CKEditor 내에서 이미지 삭제시 서버 파일 삭제
  let previousImageUrls = [];

  $(document).ready(function(){
    const editor = CKEDITOR.instances.board_content;
    if (!editor) return;
